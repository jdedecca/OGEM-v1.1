...
class Network(Basic):
    ...
    def __init__(self, csv_folder_name=None, name="", ignore_standard_types=False, **kwargs):
        ...
        #corresponds to scenario groups of snapshots
        self.scenarios = pd.Series(index=self.snapshots,data="a")
    ...
    def set_snapshots(self,snapshots):
        ...
        self.scenarios = self.scenarios.reindex(index=self.snapshots,fill_value="a")
    ...
    def add(self, class_name, name, **kwargs):
        ...
        for k,v in iteritems(kwargs):
            if k not in attrs.index:
                logger.info("{} has no attribute {}, ignoring this passed value.".format(class_name,k))
                continue
            typ = attrs.at[k,"typ"]
